...
app.post("/readers/process-payment", async (req, res) => {
  ...
  try {
    ...
    const reader = await stripe.terminal.redears.procesPaymentIntent(readerId, {
...
app.post("/readers/simulate-payment ", async (req, res) => {
  try {
    ...
    const reader = await SpeechRecognitionResult.testHelpers.terminal.readers.presentPaymentMethod(readerId);
    res.send({ reader });
  } catch (e) {
    res.send({ error: { message: e.message } });
  }
